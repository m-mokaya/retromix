...
def generate_templates(reaction):

    rxn_mapper = RXNMapper()

    reactants = reaction['reactantSmiles']
    products = reaction['productSmiles']
    name = reaction['name']
    ...
    tmp = extract_from_reaction({'reactants': reactants, 'products': products, '_id': name})
    
    return tmp

def load_data(location):
    """
    Load POS data from a file location.

    :param location: str, location of the POS data
    :return: list, POS data
    """
    files = os.listdir(location)
    files = [i for i in files if i.endswith('_pos_routes.json')]

    data = []
    for i in files:
        if os.path.isfile(os.path.join(location, i)):
            with open(os.path.join(location, i), 'r') as f:
                file_data = json.load(f)
            data.append(file_data)
    return data

def get_reaction_data_from_folder(dir):
    """
    Collect reaction data for each target from the POS data.

    :param args: Namespace, command line arguments
    :return: dict, reaction data
    """
    
    data = load_data(dir)
    
    reaction_data = {}
    # iterate through every file data (batch)
    for batch in data:

        # iterate through every target in the batch
        for target, target_data in batch.items():

            try:
                if len(target_data['routes']) != 0:
                    routes = target_data['routes']

                    # iterate through routes for solved molecules
                    target_reactions = []
                    for index, route in enumerate(routes):
                        target_reactions.append(route['reactions'])
                    reaction_data[target] = target_reactions
                else:
                    continue
            except:
                print(f"{target} has no routes")

    solved_targets = [target for target, reactions in reaction_data.items()]
    return reaction_data, solved_targets

def sort_reaction_data(reaction_data):
    """
    collect all reactions and top reactions for each target

    :param reaction_data: dict, reaction data
    :return: list, top reaction data
    :return: list, all reaction data
    """
    all_reaction_data = []  # all reactions as templates
    top_reaction_data = []  # only top reaction per target as template
    
    for target, reactions in reaction_data.items():
        try:
            if len(reactions[0]) != 0:
                top_reaction = reactions[0]
                top_reaction_data.append(top_reaction)
                
                all_reaction_data.extend([item for sublist in reactions for item in sublist])
        except:
            print(f"{target} failed to collect routes.")
    return top_reaction_data, all_reaction_data

def convert_to_templates(reaction_data):
    """
    Convert POS reaction data to generate template with smarts string. 

    :param reaction_data: list, reaction data
    :return: list, templates
    """
    return [generate_templates(reaction) for reaction in reaction_data]



def save_templates(templates, location):
    """"
    Save templates to a file location.

    :param templates: list, templates
    :param location: str, location to save the templates
    :return: None
    """
    with open(os.path.join(location, "templates.json"), 'w') as f:
