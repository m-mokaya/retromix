...
class Configuration:
    ...
    def _update_from_config(self, config: StrDict) -> None:
        ...
        for setting, value in search_config.items():
            ...
            if setting == 'optimisation_type':
                print('setting optimisation type')
                self.search.optimisation_type = value
            if setting == 'custom_templates':
                self.search.custom_templates = value
